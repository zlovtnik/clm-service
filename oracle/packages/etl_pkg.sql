...
CREATE OR REPLACE PACKAGE BODY etl_pkg AS
    ...
    BEGIN
        ...
        IF p_format = 'JSON' THEN
            ...
            FOR r IN (
                ...
                -- Use sequence for globally unique seq_num (consistent with load_record_to_staging)
                INSERT INTO etl_staging (
                    session_id, seq_num, entity_type, source_system,
                    raw_data, status, created_at
                ) VALUES (
                    p_session_id, etl_staging_seq.NEXTVAL, v_entity_type, v_source_system,
                    r.record_data, c_record_pending, SYSTIMESTAMP
                );
                v_seq_num := v_seq_num + 1; -- Keep for record count tracking only
            END LOOP;
        ELSE
    ...
    FUNCTION validate_staging_data(
        ...
        v_has_error BOOLEAN;
        v_errors VARCHAR2(4000) := ''; -- Accumulated validation error messages
    BEGIN
        FOR r IN (
            ...
            BEGIN
                ...
                IF r.entity_type = 'CONTRACT' THEN
                    ...
                    -- Additional required fields for contracts
                    IF NOT v_json_obj.has('customer_id') THEN
                        v_errors := v_errors || 'customer_id is required; ';
                        v_has_error := TRUE;
                    END IF;
                    
                    IF NOT v_json_obj.has('start_date') THEN
                        v_errors := v_errors || 'start_date is required; ';
                        v_has_error := TRUE;
                    END IF;
                ...
                ELSIF r.entity_type = 'CUSTOMER' THEN
                    ...
                    -- Additional required field for customers
                    IF NOT v_json_obj.has('name') THEN
                        v_errors := v_errors || 'name is required; ';
                        v_has_error := TRUE;
                    END IF;
                END IF;
    ...
    PROCEDURE complete_session(
        ...
        v_status VARCHAR2(20);
    BEGIN
        -- Validate session exists and is in a valid pre-completion state
        BEGIN
            SELECT status INTO v_status
            FROM etl_sessions
            WHERE session_id = p_session_id
        ...
        IF v_status NOT IN (c_session_active, c_session_processing) THEN
            RAISE_APPLICATION_ERROR(-20011, 'Cannot complete session in status: ' || v_status || '. Must be ACTIVE or PROCESSING');
        END IF;
    ...
    PROCEDURE rollback_session(
        ...
        v_cnt NUMBER;
    BEGIN
        -- Check if session exists before attempting rollback
