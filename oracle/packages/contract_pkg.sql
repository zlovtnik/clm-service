...
CREATE OR REPLACE PACKAGE BODY contract_pkg AS
    ...
    BEGIN
        ...
        -- Update contract (use NVL consistently to preserve existing values when NULL is passed)
        UPDATE contracts SET
            ...
            end_date = NVL(p_contract.end_date, end_date),
            duration_months = NVL(p_contract.duration_months, duration_months),
            auto_renew = NVL(p_contract.auto_renew, auto_renew),
            total_value = NVL(p_contract.total_value, total_value),
            payment_terms = NVL(p_contract.payment_terms, payment_terms),
            billing_cycle = NVL(p_contract.billing_cycle, billing_cycle),
            signed_at = NVL(p_contract.signed_at, signed_at),
            signed_by = NVL(p_contract.signed_by, signed_by),
            notes = NVL(p_contract.notes, notes),
    ...
    BEGIN
        ...
        -- Only consider future or same-day expirations, not already-expired contracts
        RETURN v_end_date >= TRUNC(SYSDATE) AND v_end_date <= SYSDATE + p_days_threshold;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN FALSE;
