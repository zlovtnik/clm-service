...
CREATE OR REPLACE PACKAGE BODY customer_pkg AS
    ...
    BEGIN
        -- Validate p_merge_on upfront before processing any records (fail-fast)
        IF p_merge_on NOT IN ('CUSTOMER_CODE', 'TAX_ID') THEN
            RAISE_APPLICATION_ERROR(-20001, 'Invalid merge criteria: ' || p_merge_on || '. Must be CUSTOMER_CODE or TAX_ID');
        END IF;
        ...
        IF p_match_criteria = 'TAX_ID' THEN
            OPEN p_results FOR
                SELECT tax_id, COUNT(*) as dup_count,
                       LISTAGG(id, ',' ON OVERFLOW TRUNCATE '...' WITH COUNT) WITHIN GROUP (ORDER BY created_at) as customer_ids,
                       LISTAGG(name, ' | ' ON OVERFLOW TRUNCATE '...' WITH COUNT) WITHIN GROUP (ORDER BY created_at) as names
                FROM customers
                WHERE tenant_id = p_tenant_id
