...
CREATE OR REPLACE PACKAGE BODY integration_pkg AS
    ...
    -- Autonomous transaction logger - does not affect caller's transaction
    PROCEDURE log_integration_error_autonomous(
    ...
    BEGIN
        ...
        VALUES (p_message, p_error_details, SYSTIMESTAMP);
        COMMIT;
    EXCEPTION
        WHEN OTHERS THEN
            -- Fallback to DBMS_OUTPUT if logging fails
            DBMS_OUTPUT.PUT_LINE('Log failed: ' || p_message || ' - ' || p_error_details);
    END log_integration_error_autonomous;
    
    FUNCTION generate_message_id RETURN VARCHAR2 IS
    BEGIN
        -- Convert RAW GUID to 32-character hex string
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                -- Log parse error with context using autonomous transaction (preserves caller's transaction)
                DECLARE
                    v_error_msg VARCHAR2(4000);
                BEGIN
                    v_error_msg := 'JSON parse failed: ' || SQLERRM || ' - ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE();
                    log_integration_error_autonomous('JSON parse failed for routing rules evaluation', v_error_msg);
                END;
    ...
    BEGIN
        ...
        BEGIN
            ...
            -- Append message to aggregated result using proper JSON handling
            DECLARE
                v_json_array JSON_ARRAY_T;
                v_msg_element JSON_ELEMENT_T;
            BEGIN
                IF v_current_result IS NULL THEN
                    v_json_array := JSON_ARRAY_T();
                ELSE
                    v_json_array := JSON_ARRAY_T.parse(v_current_result);
                END IF;
                
                -- Try to parse message as JSON element, fall back to string if invalid
                BEGIN
                    v_msg_element := JSON_ELEMENT_T.parse(p_message);
                    v_json_array.append(v_msg_element);
                EXCEPTION
                    WHEN OTHERS THEN
                        -- Wrap as string if not valid JSON
                        v_json_array.append(p_message);
                END;
                
                v_new_result := v_json_array.to_clob();
            END;
    ...
    BEGIN
        ...
        IF v_count > 0 THEN
            ...
            COMMIT; -- Persist dedup tracking to avoid leaving locks/open transactions
